function vl_sift(img::AbstractArray{Gray{N0f8}, 2};
  ...
  if peak_threshold >= 0 filter.peak_thresh = peak_threshold end
  if edge_threshold >= 0 filter.edge_thresh = edge_threshold end
  if norm_threshold >= 0 filter.norm_thresh = norm_threshold end
  if magnif        >= 0 filter.magnif      = magnif end
  if window_size    >= 0 filter.windowSize = window_size end
